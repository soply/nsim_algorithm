...
def sample_1D_fromClass_lesser(manifold, f_on_manifold,
    ...
    if return_original:
        s_disc, points, _, _, fval, _, _, _ = sample_1D_fromClass(manifold, f_on_manifold,
                                                n_samples, noise_level, tube = tube, var_f = var_f,
                                                return_original = return_original, args_f = args_f)
    else:
        s_disc, points, _, fval, _, _, _ = sample_1D_fromClass(manifold, f_on_manifold,
                                                n_samples, noise_level, tube = tube, var_f = var_f,
                                                return_original = return_original, args_f = args_f)
